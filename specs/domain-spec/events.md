...
| **SnapshotTaken** | `snapshot` command only — never implicit (PRD OQ5 decision: `report` warns on a dirty Assessment instead of auto-snapshotting) | Assessment exists; scoring succeeds (DI-010) | New immutable Snapshot appended with `seq = max+1`, frozen Answers, cached Score (CAP-005); zero-answer snapshot warns (DEC-004) |
