...
| DI-003 | **Floor rule:** the headline Maturity Band cannot exceed the **floor-cap band — defined positionally as the second-lowest band in the catalog's ordered band list** ("Developing" in the bundled CPG catalog) — while any Critical-tier Goal (effective tier, after overrides) is `Not Implemented` or `Unknown`. The rule is positional so it survives catalog band renames (DI-008). | Score | Scoring marks `floor_rule_triggered=true`, lists the gating `critical_gaps[]`, and every display surface shows why the band is capped. |
...
| DI-012 | Percentage formula: `percent = Σ(weight_g × level_value_g) / Σ(weight_g)` over applicable (non-N/A) Goals, computed per Function and overall. Level values are **exact rationals**: NI=0, PI=1/3 (displays as 0.33), LI=2/3 (displays as 0.67), FI=1, Unknown=0; engine arithmetic is rational/fixed-point, never binary floating point. | Score | Single formula for all roll-ups; any alternative aggregation in any surface is a defect. |
